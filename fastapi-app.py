...
def upload_new_files_to_drive():
    ...
    if st.session_state.uploaded_files:
      try:
          ...
          # save the vetorstore_db to drive
...
def from_drive_vectorstore():
    ...
    Callback function to handle Google Drive file processing
    """
    if st.session_state.category_selection and st.session_state.uploaded_files_to_drive:
        try:
          temp_dir = tempfile.mkdtemp() 
...
with st.sidebar:
  ...
  if st.session_state.upload_option == "Local File":
    ...
    subfolders = drive_manager.list_subfolders(parent_folder_id)
    for subfolder in subfolders:
        categories.append(subfolder['name'])
    ...
    if "uploaded_files_to_drive" not in st.session_state:
        st.session_state.uploaded_files_to_drive = None
